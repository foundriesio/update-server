...
func RegisterHandlers(e *echo.Echo, ca *DeviceCa, storage *storage.Storage, userStorage *users.Storage, a auth.Provider) {
	...
	upd.POST("/:tag/:update", h.updateCreate, requireScope(users.ScopeUpdatesRU),
		gzipContentTypeAsContentEncoding, middleware.Decompress())
	upd.DELETE("/:update", h.updateDelete, requireScope(users.ScopeUpdatesD))
	upd.GET("/:update/summary", h.updateSummary, requireScope(users.ScopeUpdatesR))
	upd.GET("/:update/query", h.updateQuery, requireScope(users.ScopeUpdatesR))
	upd.GET("/:update/tuf", h.updateGetTuf, requireScope(users.ScopeUpdatesR))
	...
	upd.GET("/:update/tail", h.updateTail, requireScope(users.ScopeUpdatesR))
