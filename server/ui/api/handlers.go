...
func RegisterHandlers(e *echo.Echo, ca *DeviceCa, storage *storage.Storage, a auth.Provider) {
	...
	upd.GET("/:tag/:update/report", h.updateReport, requireScope(users.ScopeUpdatesR))
	...
	upd.GET("/:tag/:update/rollouts/:rollout/report", h.updateRolloutReport, requireScope(users.ScopeUpdatesR))
