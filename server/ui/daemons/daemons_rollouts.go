...
func (d *daemons) rolloutWatchdog() daemonFunc {
...
func (d *daemons) processJournal() (success bool) {
	log := context.CtxGetLog(d.context)
	success = true
	for line, err := range d.storage.ReadRolloutJournal() {
		if err != nil {
		...
		if rollout, err := d.storage.GetRollout(tag, updateName, rolloutName); err != nil {
			...
			log.Error("failed to process rollout file", "error", err, "path", line)
			success = false
		} else if !rollout.Commit {
			// Rollout file present but not committed - commit it now.
			if err = d.storage.CommitRollout(tag, updateName, rolloutName, rollout); err != nil {
				log.Error("failed to commit rollout", "error", err, "path", line)
				success = false
			}
		}
	}
