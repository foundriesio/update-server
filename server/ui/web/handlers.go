...
func RegisterHandlers(e *echo.Echo, storage *users.Storage, authProvider auth.Provider, branding Branding, brandingDir string, pages PageBuilder) {
	...
	e.GET("/configs/device/:uuid/apps", func(c echo.Context) error {
		return c.Redirect(http.StatusMovedPermanently, "/devices/"+c.Param("uuid")+"/configs/apps")
	})
	...
	e.GET("/devices/:uuid/updates", h.devicesUpdatesGet, h.requireSession, h.requireScope(users.ScopeDevicesR))
	e.GET("/devices/:uuid/updates/:update", h.devicesUpdateGet, h.requireSession, h.requireScope(users.ScopeDevicesR))
	...
	e.GET("/devices/:uuid/configs/apps", h.configsDeviceItemApps, h.requireSession, h.requireScope(users.ScopeDevicesR))
	...
	e.GET("/updates/:name", h.updatesGet, h.requireSession, h.requireScope(users.ScopeUpdatesR))
	e.GET("/updates/:name/tail", h.updatesTail, h.requireSession, h.requireScope(users.ScopeUpdatesR))
	e.GET("/updates/:name/rollouts/:rollout", h.updatesRollout, h.requireSession, h.requireScope(users.ScopeUpdatesR))
	e.GET("/updates/:name/rollouts/:rollout/tail", h.updatesRolloutTail, h.requireSession, h.requireScope(users.ScopeUpdatesR))
