...
# Usage: gen_pki.sh <DATA_DIR> <DG_SAT_BIN> [HOSTNAME] [FACTORY]

DATA_DIR=$1
DG_SAT_BIN=$2
HOSTNAME=${3:-dg-satellite}
FACTORY=${4:-e2e-factory}

if [ -z "$DATA_DIR" ] || [ -z "$DG_SAT_BIN" ]; then
    echo "Usage: $0 <data_dir> <dg_sat_bin> [hostname] [factory]"
    exit 1
fi

mkdir -p "${DATA_DIR}/certs"

echo "## Generating TLS CSR via dg-sat"
"$DG_SAT_BIN" --datadir "$DATA_DIR" create-csr --dnsname "$HOSTNAME" --factory "$FACTORY"
...
cd "$DATA_DIR"
"$DG_SAT_BIN" --datadir "$DATA_DIR" sign-csr \
