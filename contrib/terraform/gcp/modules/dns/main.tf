...
locals {
  ...
  # A map keyed by DNS name naturally collapses to one record when the UI and
  # the gateway share a hostname, which is exactly the Caddy topology's case
  # (single instance IP, single name).
  records = {
