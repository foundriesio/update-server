...
def test_settings_api_token_and_audit_log(page, dg_satellite_server):
    ...
    dialog.locator("input[type='date']").fill(tomorrow)
    ...
    page.wait_for_load_state("networkidle")
    assert "Token created" in page.content(), f"Audit log does not show API token creation event:\n{page.content()}"
